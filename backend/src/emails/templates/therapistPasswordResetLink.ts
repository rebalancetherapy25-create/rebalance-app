...
export const therapistPasswordResetLinkEmail = (options: { resetUrl: string }) => {
    const safeUrl = esc(options.resetUrl);

    const body = `
      <h1 class="h1">Reset your portal password</h1>
      <p class="p">We received a request to reset your ReBalance Therapist Portal password. Click the button below to set a new one.</p>
    ...
    return {
        subject: 'Reset your Rebalance Therapist Portal password',
        html: emailLayout({
            title: 'Reset your password',
            preheader: 'Use this secure link to set a new ReBalance Therapist Portal password',
            body,
        }),
    };
};
